describe("Authentication workflow", () => {
    ...
    context("Authenticate", () => {
        ...
        afterEach(() => {
            cy.wait("@googleApi")
                .its("url")
                .should("include", "emailLinkSignin");
        });

        it("Should prompt for an email if not available from local storage", () => {
            cy.visit("localhost:3000/authenticate?apiKey=0&oobCode=1&mode=signIn&lang=en", {
                onBeforeLoad(win) {
        ...
        it("Should use the email in local storage if its available", () => {
            ...
            cy.visit("localhost:3000/authenticate?apiKey=0&oobCode=1&mode=signIn&lang=en", {
                onBeforeLoad(win) {
                    win.localStorage.setItem("emailForAuthenticateHandler", "cole@example.org");
                    cy.stub(win, "prompt").returns("cole@example.org");
                }
            });
            cy.window()
